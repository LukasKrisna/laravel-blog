<nav class="bg-gray-800/50" x-data="{ mobileOpen: false, profileOpen: false }" aria-label="Global">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div class="flex h-16 items-center justify-between">
        ...
        <div class="md:hidden">
          <!-- Mobile menu button -->
          <button type="button" @click="mobileOpen = !mobileOpen" class="relative inline-flex items-center justify-center p-2 rounded-md text-white bg-gray-700 hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800">
            <span class="sr-only">Open main menu</span>
            <!-- Hamburger icon -->
            <svg x-show="!mobileOpen" class="block h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <div x-show="mobileOpen" class="block md:hidden">
      ...
      <div class="border-t border-white/10 pt-4 pb-3">
        ...
        <div class="mt-3 space-y-1 px-2">
          ...
          <form method="POST" action="/logout" class="w-full">
            @csrf
            <button class="w-full block px-3 py-2 text-base font-medium text-gray-400 hover:bg-white/5 focus:bg-white/5 focus:outline-hidden cursor-pointer text-left rounded-md">Log out</button>
          </form>
        </div>
        @else
        <div class="mt-3 space-y-1 px-2">
